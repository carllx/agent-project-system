...
正常恢复不得扫描全部 pre-send Conversation。后续 Review round 必须携带上一轮 Product state；只有其中 exact `delivery_conversation_id == target_conversation_id`、Delivery establishment 已记录唯一 marker，且对该 exact ID 的 bounded read-only detail 仍能找到上一轮 Work Item/Message marker，才建立 persisted known-target continuation。发送前 exact-ID verification 最多尝试三次，只对 timeout、nonzero 或 unparseable read 继续下一次同-ID尝试，成功立即停止；可读但 marker 不唯一则立即 fail closed。该路径不查询 recent history，也不依赖当前 active tab；history 只服务 discovery，不能反向否决已验证 exact target。尝试耗尽、exact target 不可访问或 marker 不唯一时在 write 前安全 `BLOCKED`，不得猜测其他 Conversation 或自动转 Manual Relay。existing-target write 后的 recovery 仍优先 exact-detail 持久 target，即使 post-send Browser current 位于其他 Conversation；new-session 发送后 status 若给出 exact current ID，恢复只对该 exact ID 执行最多一次 detail。status 无 exact ID 时，才刷新同一 bounded history window、排除基线并要求唯一 `NEW_CANDIDATE_DIFF`。只搜索精确 `WORK_ITEM_ID` 与 `MESSAGE_ID`，且必须恰好命中一个 user marker；returned identity、current identity 和 history candidate 都只是 observation，不能单独建立 delivery。当前发送后目标若属于发送前 ID，或 existing-target delivery 与 target 不同，且 exact marker 命中，则为 `MISROUTED_DELIVERY`；找不到或 marker 重复则保持 `DELIVERY_UNKNOWN` 并永久禁止该 Message ID 重发。
