...
普通同步 Shell 命令一旦返回 `exit code`、`stdout` 和 `stderr` 即已完成。若 Work Order 只授权这一条命令，实验 Agent 必须在当前回合立即整理最终报告并结束；其后调用 `schedule`、`sleep`、timer、`wait`、延迟轮询或等待通知，统一分类为 `HARD_FAILURE: TEST_PROTOCOL_VIOLATION`，失败类型为 `UNAUTHORIZED_IDLE_WAIT`。只有工具明确返回 `RUNNING`、`PENDING`、`PROCESS_STILL_ACTIVE` 或 `JOB_ID_WITH_INCOMPLETE_RESULT`，且 Work Order 明确授权轮询、等待仍在显式预算内、存在可验证的后台进程或 Job ID 三项同时成立时，才允许等待或轮询。同步结果不满足该例外。

实验默认 `MAX_IDLE_WAIT_SECONDS=0`、`MAX_SCHEDULE_CALLS=0`、`MAX_POLL_ATTEMPTS=0`；只能由具体 Work Order 显式覆盖。Shell、`schedule`、`sleep`、timer、轮询、文件搜索、源码搜索和日志搜索全部从实验开始起计入动作，不得把正式命令前的发现动作排除。报告至少包含 `PLACEHOLDER_VALIDATION_PERFORMED`、`UNRESOLVED_PLACEHOLDERS`、`SCHEDULE_CALL_COUNT`、`IDLE_WAIT_SECONDS`、`POLL_ATTEMPT_COUNT`、`SYNCHRONOUS_COMMAND_COMPLETED`、`TERMINATED_IMMEDIATELY_AFTER_RESULT`，并附外部命令总数、实验动作总数和分类型计数；三个等待计数默认均为零。报告不得以 `standing by`、`waiting for timer`、`I will report later` 或 `等待下一次状态检测` 结束，必须在当前回合给出最终结果。
