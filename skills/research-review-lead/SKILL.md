...
Within the same Wrapper process, record the bounded history baseline before `new`; treat the `new` command result only as navigation-attempt Evidence, then verify the actual Browser URL is ChatGPT root or `/new` and require the shared read classifier to return `EMPTY`. A timeout, nonzero result, or missing result row from `new` does not authorize a write, but it also does not bypass the authoritative post-new status/read verification. Only after that verification may the Wrapper invoke `opencli chatgpt send` once. If a successful NEW first-write still has immediate status `/new` or root, observe only `chatgpt status` for at most 30 seconds and at most 10 attempts. Do not assume an external status process completes in one second. This read-only phase does not consume the normal 9-command, 60-second operation, recovery, detail, or write budgets; its elapsed time is explicitly excluded from the old operation budget, so the qualifying operation's physical wall-clock can increase by at most 30 seconds. The first exact `/c/<id>` is only current/navigation Evidence. Require one exact current-page Work Item/Message marker before establishing delivery or promoting target. Deadline, attempt exhaustion, actual status error, invalid identity, missing marker, or duplicate marker remains `DELIVERY_UNKNOWN` and permanently forbids same-ID resend. Existing-target continuation does not use this navigation wait. Never use `ask --new`, never require the user to open `/new`, and never split creation and sending into separate formal experiments. The standalone `prepare-new` command remains only for backward-compatible diagnostics and is not a prerequisite or acceptance step.
...
Recovery saves a bounded pre-send history baseline, performs one send, and reads post-send status. The NEW successful-write `/new` or root case is handled exclusively by `POST_SEND_NAVIGATION_WAIT`; its deadline does not fall through to history recovery. Other authorized recovery paths may refresh the same bounded history window once, compute IDs absent from the baseline, and perform at most one detail check against the strongest bounded target. Search only exact Work Item ID and Message ID, stop on the first hit, and retain no unrelated conversation body. Never scan all pre-send Conversations, enlarge the candidate limit, or use navigation polling as a recovery substitute.
